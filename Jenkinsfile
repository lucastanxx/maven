pipeline {
    ...
    stages {
        stage ('Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/lucastanxx/maven.git'
            }
        }
        stage ('Build') {
            steps {
    ...
    post {
        always {
            ...
            recordIssues enabledForFailure: true, tools: [mavenConsole(), java(), javaDoc(), checkStyle(), spotBugs(pattern: '**/target/findbugsXml.xml'), cpd(pattern: '**/target/cpd.xml'), pmdParser(pattern: '**/target/pmd.xml')]
        }
    }
}
